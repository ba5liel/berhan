...
import 'package:berhan/models/carousal_item_model.dart';
import 'package:berhan/viewmodels/home_camera_viewmodel.dart';
import 'package:berhan/views/core/base_view.dart';
import 'package:berhan/views/pages/random_particles_view.dart';
import 'package:berhan/views/pages/search_image_view.dart';
import 'package:berhan/views/widgets/carousal_slider_widget.dart';
...
final List<CarousalItemModel> _carousallist = [
  CarousalItemModel(Icons.speaker,"To speech"),
  CarousalItemModel(Icons.library_books,"To Text"),
  ...
  void selectedpage(int pagindex) async{
    ...
    try {
     ...
     final path = await cameracontroller.takePicture();
      ...
      var decodedImage = await decodeImageFromList(File(path.path).readAsBytesSync());

     await Navigator.push(_context, PageTransition(type: PageTransitionType.fade, child:  SearchImageView(imagePath: path.path,islandscape: decodedImage.width > decodedImage.height,)));
